function baidu_map(id, data) {
    ...
    if (data) {
        ...
        var point = new BMap.Point(121.527644, 31.292536);
        doit(point);
    ...
    function doit(point) {
        ...
        if (point) {
            ...
            $("div[mapdiv='" + id + "'] #lng").val(point.lng);
            $("div[mapdiv='" + id + "'] #lat").val(point.lat);
            ...
            myGeo.getLocation(point, function (result) {
                if (result) {
                    ...
                    $("div[mapdiv='" + id + "'] #suggestId").val(result.address);
                }
            });
            ...
            marker2.addEventListener("dragend", function (e) {
                $("div[mapdiv='" + id + "'] #lng").val(e.point.lng);
                $("div[mapdiv='" + id + "'] #lat").val(e.point.lat);
                //document.getElementById('lat').value = e.point.lat;
                //document.getElementById('lng').value = e.point.lng;
                myGeo.getLocation(new BMap.Point(e.point.lng, e.point.lat), function (result) {
                    if (result) {
                        //jQuery('suggestId').value = result.address;
                        $("div[mapdiv='" + id + "'] #suggestId").val(result.address);
            ...
            map.addEventListener("dragend", function showInfo() {
                ...
                myGeo.getLocation(new BMap.Point(cp.lng, cp.lat), function (result) {
                    if (result) {
                        $("div[mapdiv='" + id + "'] #lng").val(cp.lng);
                        $("div[mapdiv='" + id + "'] #lat").val(cp.lat);
                        $("div[mapdiv='" + id + "'] #suggestId").val(result.address);
    ...
    function loadmap() {
        ...
        var city = $("div[mapdiv='" + id + "'] #suggestId").val();
    ...
    function setarrea(address, city) {
        //jQuery('suggestId').value = address;
        $("div[mapdiv='" + id + "'] #suggestId").val(address);
        //$('city').value=city;
        window.setTimeout(function () {
            loadmap();
        }, 2000);
    ...
    jQuery("div[mapdiv='" + id + "'] #suggestId").change(function () { loadmap(); })
    jQuery("#positioning").click(function () { loadmap(); });
