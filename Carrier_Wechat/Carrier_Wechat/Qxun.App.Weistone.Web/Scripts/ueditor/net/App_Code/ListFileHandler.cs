...
public class ListFileManager : Handler
{
    ...
    public override void Process()
    {
        ...
        try
        {
            ...
            PathToList = System.Configuration.ConfigurationManager.AppSettings["SaveFilePath"] + "/" + catelog;
            var localPath = Server.MapPath(PathToList);
            buildingList.AddRange(Directory.GetFiles(localPath, "*", SearchOption.AllDirectories)
                .Where(x => SearchExtensions.Contains(Path.GetExtension(x).ToLower()))
                .Select(x => string.Format("{0}{1}{2}", UrlBuilder.WeiSitePath, PathToList.TrimStart('~'), x.Substring(localPath.Length).Replace("\\", "/"))));
