    ...
    $.ThreeSixty = function (el, options) {
        ...
        */
        base.loadImages = function () {
            ...
            image = $('<img>').attr('src', imgUrls[AppCongif.loadedImages]).addClass('previous-image').appendTo(li);
            ...
            $(image).load(function () {
                var img = new Image();
                img.src = frames[AppCongif.loadedImages].attr('src');
                if (img.complete) {
                    var top = 0;
                    var imgRealHeight = AppCongif.width/img.width*img.height;
                    if (imgRealHeight<AppCongif.height){
                        top = (AppCongif.height-imgRealHeight)/2;
                    };
                    frames[AppCongif.loadedImages].css({
                        top: top + 'px'
                    });
                }
                else {
                    img.onload = function () {
                        var top = 0;
                        var imgRealHeight = AppCongif.width / img.width * AppCongif.height;
                        if (imgRealHeight < AppCongif.height) {
                            top = (AppCongif.height - imgRealHeight) / 2;
                        };
                        frames[AppCongif.loadedImages].css({
