...
document.writeln("	<div id=\"title\" class=\"title\">消息提醒<\/div>");
...
document.writeln("<\/div>");
document.writeln("<div class=\"dialogCover\"></div>");
...
function alert(title, url) {
    flag = "";
    $(".dialogCover").css({ "display": "block", "position": "fixed", "left": "0", "top": "0", "width": "100%", "height": "100%", "z-index": "1000", "background": "rgba(0, 0, 0, 0.2)" });
    url = url || "";
    $("#windowcenter").slideToggle("slow",function () {
    });
    $("#txt").html(title);
    if (url!="") {
        flag = url;
